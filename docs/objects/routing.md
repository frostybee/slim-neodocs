...
if you add the  [Output Buffering Middleware](../middleware/output-buffering).

Please note that since Slim 4, you must return a `Psr\Http\Message\ResponseInterface` object.

### Closure binding

If you use a [dependency container](../concepts/di) and a `Closure` instance as the route callback, the closure's state is bound to the `Container` instance. This means you will have access to the DI container instance _inside_ of the Closure via the `$this` keyword:
